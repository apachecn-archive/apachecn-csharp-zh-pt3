...
一些应用和实现使用了`Deconstruct`方法。此方法的目的是将属性解构为离散变量。要使用位置模式，`Deconstruct`方法应该是可访问的，这样我们将能够查看表征对象的属性，并使用这些属性来生成模式。清单 [5-8](#PC8) 显示了`CryptoAlgorithms`类，它包括一个`Deconstruct`方法，以便为`prvKey`(私钥)和`pubKey`(公钥)创建一个离散变量。清单 [5-8](#PC8) 中的代码展示了位置模式在密码学领域的应用。结果如图 [5-5](#Fig5) 所示。
